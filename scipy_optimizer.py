...
from tensorflow.python.keras import backend as K  # pylint: disable=import-error
...
class ScipyOptimizer(object):
    ...
    def get_power_consumption(self, ref_img, pre_img):
        ref_img_y = rgb2ycbcr(ref_img)[:, :, 0]
        pre_img_y = rgb2ycbcr(pre_img)[:, :, 0]
        return np.sum(np.power(pre_img_y, 2.2)) / np.sum(np.power(ref_img_y, 2.2))
    ...
    def _validate(self, x, val_generator, state):
        ...
        for batch_index in range(n_steps):
            inputs, eta = val_generator[batch_index]
            pre_img, diff = self.generator_model.predict(inputs)
            pre_img = np.clip((0.5 * pre_img + 0.5)[0, :, :, :], 0, 1)*255
            ori_img = ((0.5 * inputs + 0.5)*255)[0, :, :, :]
            p = self.get_power_consumption(ori_img, pre_img)
            ssim = compare_ssim(ori_img, pre_img, multichannel=True)
            if state['epoch'] == 0:
                self.ssim = 0
            elif eta-0.01 <= p <= eta+0.01 and self.ssim < ssim:
                self.pre_img_final = pre_img
                self.ssim = ssim
                imsave(self.target_path + "/pre_img.png", self.pre_img_final)
