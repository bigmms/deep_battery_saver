...
class ScipyOptimizer(object):
    ...
    def getPC(self, img, A):
        img_ycbcr = rgb2ycbcr(img)
        img_y = img_ycbcr[:, :, 0]
        A_ycbcr = rgb2ycbcr(A)
        A_y = A_ycbcr[:, :, 0]
        p = np.sum(np.power(A_y, 2.2)) / np.sum(np.power(img_y, 2.2))
        return p
    ...
    def _validate(self, x, val_generator, state):
        ...
        for batch_index in range(n_steps):
            ...
            fake_img, diff = self.generator_model.predict(inputs)
            fake_img = (0.5 * fake_img + 0.5)
            img = fake_img[0, :, :, :]
            img = np.clip(img, 0, 1)

            A = img*255
            p = self.getPC(((0.5 * inputs + 0.5)*255)[0,:,:,:], A)
            if p>=0.79 and p<=0.81:
                imsave("./saved_graph/fake_img_%06d_%04f.png"% (state['epoch'], p), img)
